...
def batch_lookup(M, idx, vector_output=True):
    ...
    if sample_size == 1 and vector_output:
        if len_size == 2:
            ...
            samples = torch.cat((torch.gather(M[:, :, 0], 1, idx), torch.gather(M[:, :, 1], 1, idx), torch.gather(M[:, :, 0], 1, idx)),dim=1).view(-1, 3)
    else:
        if len_size == 2:
            samples = torch.gather(M, 1, idx)
        else:
            samples = torch.cat(
                (torch.gather(M[:, :, 0], 1, idx), torch.gather(M[:, :, 1], 1, idx), torch.gather(M[:, :, 0], 1, idx)),
                dim=1).reshape(idx.size() + (3,))
            # TODO Check path
    return samples
